package boomiui.editor
{
	...
	{		
		...
		private var m_checkBox : Check;
		
		public function CheckBoxEditor()
		{
			m_type = "CheckBox";
			
			m_checkBox = new Check();
			m_checkBox.addEventListener(FeathersEventType.CREATION_COMPLETE , onCreateComponent);
			addChild(m_checkBox);
//			CheckBoxGroup.getInstance().addButton(m_checkBox);
		 ...
		 * 设置文本的偏移坐标 默认是按钮内部的0 
		 */
		public function get tfOffsetY():int{return m_checkBox.labelOffsetY;}
		public function set tfOffsetY(value:int):void{m_checkBox.labelOffsetY = value;}
		
		/**
		 * 设置文本的偏移坐标 默认是按钮内部的0
		 */
		public function get tfOffsetX():int{return m_checkBox.labelOffsetX;}
		public function set tfOffsetX(value:int):void{m_checkBox.labelOffsetX = value;}
