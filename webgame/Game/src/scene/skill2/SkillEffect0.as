package scene.skill2
{
	 ...
	 * 施法特效
	 */
	public class SkillEffect0 extends WorldSprite implements ISkillEffect
	{
		...
		public static const SKILL_EFFECT_X:int = 0;
		/**
		 * 内容的默认播放次数
		 */
		public static const D_AS_MOVIE_PLAY_COUNT:int=1;
		
		private var m_nPlayTime:int = 0;
		...
		{
			var m_info:Array=SkillEffectManager.instance.getFXAndAngle(1, 1);
			if (m_info[0] != null)
				k_roleFX=m_info[0];
	...
	//项目转换	return Lib.getObj(LibDef.PUB_SKILL, skill.toString());
		...
		public function One_CreateEffect():void
		{
			...
			var d:DisplayObject=SkillEffectManager.instance.getPool(this.skill, 1, this.skill, this.targetInfo.src_sex, this.targetInfo.srcid, angle - offsetAngle,kingRoleFX,playTime);
		...
		private function Two_Sub_RefreshFx(k:IGameKing):void
		{
			...
			for (i=0; i < this.numChildren; i++)
			{
				...
				SkillEffectManager.instance.AdjustBodyXY(d, "", this.targetInfo, 0, this.skill);
...
//				}
		...
		{
			//
			var useTweenLite:Boolean=false;

			moveTime=m_nPlayTime / 1000;
			//check
