package com.bellaxu.mgr
{
	...
	public final class MusicMgr
	{
		...
		private static var isMusicOff:Boolean = false;
		//当前的背景音乐路径
		...
		public static function set musicOff(bo:Boolean):void
		{
			...
			UI_index.indexMC_mrt_smallmap["btnCloseMusic1"].visible = isMusicOff;
			UI_index.indexMC_mrt_smallmap["btnCloseMusic2"].visible = !isMusicOff;
...
//				return;
			if (isMusicOff || waveUrl == "" || waveUrl == null || waveUrl == "undefined")
				return;
		...
		{
			if (isMusicOff || curMusicUrl == "" || curMusicUrl == null)
				return;
			if (null != backChannel)
			{
				backChannel.removeEventListener(Event.SOUND_COMPLETE, BacksoundCompleteHandler);
