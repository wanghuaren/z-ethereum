package com.bellaxu.struct
{
	 ...
	 * @author BellaXu
	 */
	public class BitmapInfo implements IBitmapInfo
	{
...
/**
		public function get clone():IBitmapInfo
		{
			var bitmapInfo:IBitmapInfo=new BitmapInfo();
			bitmapInfo.bitmapData=_bitmapData.clone();
			bitmapInfo.x=_x;
			bitmapInfo.y=_y;
			bitmapInfo.mark=_mark;
			bitmapInfo.originX=_originX;
			bitmapInfo.originY=_originY;
			bitmapInfo.center=_center;
			bitmapInfo.centerToFoot = _centerToFoot;
			return bitmapInfo;
		}
*/
		...
		{
			_bitmapData=value;
			if (_bitmapData != ResDef.defautlBody)
			{
		...
		/**
		 * warring
		 * 传址给loadable 所以在不确定Bitmapdata无用之前，些处不能够销毁
		**/
		public function destory(gc:Boolean=false):void
		{
			_isDestroyed = true;
			
			if(gc&&this.bitmapData)
			...
			this.mark = null;
			var key:String;
			for (key in m_nResLoadedDic)
			{
				delete m_nResLoadedDic[key];
			}
			key = null;
