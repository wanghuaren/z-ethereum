...
//		public static const DEFAULT_PLAY_TIME_PER_FRAME:int = 100;
		/** 播放时间间隔   **/		
		public var movie:Vector.<IBitmapInfo> = null;
		...
		public var curBitmap:Bitmap = null;
...
//			if (mcName.indexOf("S")!=-1)
			if (this.currentBitmapData == null || this.currentBitmapData == ResDef.defautlBody || actionIndex != currentActionIndex)
			{
				this.nextFrame();
			}
		}
		...
		override public function addEventListener(type:String, listener:Function, useCapture:Boolean=false, priority:int=0, useWeakReference:Boolean=false):void
		{
			...
			_listers.put(type, {type:type, listener:listener});
		}
		...
		private function getBeginEnd(frame:String):void
		{
			...
			var _bitmapInfo:IBitmapInfo = null;
			var i:int = 0;
			for (i = 0; i < len; i++)
			{
				_bitmapInfo = movie[i];
		...
		private var m_nResLoadedCache:Hash = new Hash();
		...
		{
			var _bitmapInfo:IBitmapInfo = movie[_frameIndex];
			var m_nIsResLoaded:Boolean;
			var actStr:String = _bitmapInfo.act;
			if (m_nResLoadedCache.take(actStr)==null&&_bitmapInfo)
			{
				m_nIsResLoaded = ResMcMgr.isResLoadedInAct(mcName,_bitmapInfo.act);
				if (m_nIsResLoaded)
				{
					m_nResLoadedCache.put(actStr,1);
				}
				else
				{
					return false;
...
//				if (mcName.indexOf("S")==-1 && isPart)
//				else
				
				if (isResLoaded)
				{
					curBitmap.bitmapData = _bitmapInfo.bitmapData;
//					if (mcName == "Main_100001W")
				}
				else
				{
					curBitmap.bitmapData = ResDef.defautlBody;
					curBitmap.y = -120;
				}
			}
			else
...
//							child._frameIndex = _frameIndex;//附属部件动作帧与主模型动作统一
		...
		private var m_nMain_10604001Offset:Array = [,[-15,0],[-10,-5],[-15,-15],[-20,-10],[-15,-10],[-10,-10],[-15,-10],[-5,-10]];
		...
		public function get currentBitmapInfo():IBitmapInfo
		{
			if(!_undisposed_)
				return null;
		...
		public function close():void
		{
			...
			//if (this.curBitmap.parent==null)
			//	this.addChild(this.curBitmap);
						this.curBitmap = null;
			for (var key:Object in _listers)
			{
				var obj:Object = _listers.take(key);
			...
			this.act = "D1";
			this.dir = "F1";
			this.m_nResLoadedCache.dispose();
			this.m_nResLoadedCache = new Hash();
				...
				if (layer == 1)
				{
					...
					for(var i:int = this.numChildren - 1;i > -1;i--)
					{
						...
						if(child.name.indexOf("horse") > -1)
						{
							...
							ResMcMgr.release(child as ResMc);
						}
					}
					if(act == "D10")
						act = "D1";
...
//					setTimeout(gotoAndPlay, 33, act + dir);
				...
				if (layer == 0)
					wing = value;
				if (_mattrix[layer] != null && _mattrix[layer].parent != null)
				{
					_mattrix[layer].parent.removeChild(_mattrix[layer]);
					ResMcMgr.release(_mattrix[layer] as ResMc);
				}
				_mattrix[layer] = null;
			}
		}
		...
		public function get nTick():Number
		{
			...
			//更新帧速率
			this.m_nTick = fps / GameData.curFps;
			return m_nTick;
		}
...
//					return 0;
		...
		public function set wing(value:ResMc):void
		{
			...
			if (value == null)
			{
				if (wing != null && wing.parent != null)
				{
					wing.stop();
					ResMcMgr.release(wing);
