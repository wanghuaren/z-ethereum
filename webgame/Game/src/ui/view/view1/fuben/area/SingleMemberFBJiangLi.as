package ui.view.view1.fuben.area
{
	...
	public class SingleMemberFBJiangLi extends UIWindow
	{
		private static var sipl:Vector.<StructIntParamList2>;
		private static var callbacktype:int;
		private var daoJiShi:int;
		
		public function SingleMemberFBJiangLi(value:Vector.<StructIntParamList2>,_callbacktype:int)
		{
			sipl = value;
			callbacktype = _callbacktype;
			super(getLink("win_hua_shan_lun_jian"));
		}
		...
		private function showValue() : void {
			if(sipl[3].intparam==2){
				...
				mc["title"].gotoAndStop(4);
				GameMusic.playWave(WaveURL.ui_huodong_failt);
			}
			//
			mc["tf_0"].htmlText = sipl[5].intparam.toString();
			mc["tf_1"].htmlText = sipl[4].intparam.toString();
			mc["tf_2"].htmlText = sipl[6].intparam.toString();
