package ui.view.view1.fuben.area
{
	...
	public class PkJiangLi  extends UIWindow
	{
		...
		public function PkJiangLi(value:Vector.<StructIntParamList2>,_callbacktype:int)
		{
			sipl=value;
			callbacktype = _callbacktype;
			super(getLink("win_hua_shan_lun_jian_xin"));
			
		}
		
		private static var _instance:PkJiangLi=null;
		
		public static function instance(value:Vector.<StructIntParamList2>,_callbacktype:int):PkJiangLi
		{
			if (null == _instance)
			{
				_instance=new PkJiangLi(value,_callbacktype);
			}
			else
			{
				sipl=value;
		...
		private function showValue():void
		{
			...
			mc["tf_1"].htmlText = sipl[1].intparam.toString();
...
//			sprite["uil"].source=itemData.icon;
		...
		{
			PkJiangLi.instance(_sipl,_callbacktype).open();
		}
