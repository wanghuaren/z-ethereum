package ghostcat.algorithm.traversal
{
	...
	public class SimpleAStar
	{
		 ...
		 */		
		public function SimpleAStar(p_mapTileModel :IMapModel)
		{
		 ...
		 * 开始寻路
		 */		
		public function find(start:Point,end:Point, path :Array,maxTry :int = 5000) : int
		{
			var p_startX : int = start.x;
			var p_startY : int = start.y;
			var p_endX : int = end.x;
			var p_endY : int = end.y; 
			var tempStartTime:int = getTimer();
			this.initLists();
			this.m_openCount = 0;
			this.m_openId = -1;
			...
			while (this.m_openCount > 0)
			{
				...
				{
					 this.getPath(p_startX, p_startY, currId,path);
					 return 0;
				}
		 ...
		 */		
		private function getPath(p_startX : int, p_startY : int, p_id: int,arr :Array) : void
		{
			var noteX : int = this.m_xList[p_id];
			var noteY : int = this.m_yList[p_id];
