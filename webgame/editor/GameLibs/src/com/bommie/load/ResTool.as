...
//				return ResPreLoader.LOAD_QUEUE[ResPreLoader.currentState][ResPreLoader.currentIndex];
		...
		{
			ResMcMgr.instance.leadRoleSkin = list;
		}
		...
		{
			return ResMcMgr.instance.getByXml(xmlUrl);
		}
		
		/**
		 * 加载资源
		 */
		public static function load(url:String, onComplete:Function = null, onProgress:Function = null, onError:Function = null, domain:ApplicationDomain = null, priority:uint = 0, format:String = URLLoaderDataFormat.BINARY, ver:int = 0):void
		{
		...
		{
			ResLoader.getInstance().disposeRes(url,false);
		}
...
//		}
		...
		{
			return RoleXMLPool.instance.getXMLByPath(path);
		}
		...
		{
			var xml:XML = RoleXMLPool.instance.getXMLByPath(path, lib);
			if(xml)
				return xml;
			...
			if (domain && domain.hasDefinition(path2))
			{
				...
				xml = XML(new cl());
				RoleXMLPool.instance.saveLibXml(lib, path, xml);
				return xml;
			}
			return null;
		}
